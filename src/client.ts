...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env['REMOTE_SAVANT_MCP_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['REMOTE_SAVANT_MCP_LOG'] or 'warn' if it isn't set.
   */
...
export class RemoteSavantMcp {
  ...
  constructor({
    ...
    if (apiKey === undefined) {
      throw new Errors.RemoteSavantMcpError(
        "The REMOTE_SAVANT_MCP_API_KEY environment variable is missing or empty; either provide it, or instantiate the RemoteSavantMcp client with an apiKey option, like new RemoteSavantMcp({ apiKey: 'My API Key' }).",
      );
    }
    ...
    this.timeout = options.timeout ?? RemoteSavantMcp.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('REMOTE_SAVANT_MCP_LOG'), "process.env['REMOTE_SAVANT_MCP_LOG']", this) ??
      defaultLogLevel;
  ...
  static RemoteSavantMcp = this;
  static DEFAULT_TIMEOUT = 60000; // 1 minute

  static RemoteSavantMcpError = Errors.RemoteSavantMcpError;
