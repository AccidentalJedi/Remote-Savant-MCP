...
import { APIPromise } from 'remote-savant-mcp/core/api-promise';

import util from 'node:util';
import RemoteSavantMcp from 'remote-savant-mcp';
import { APIUserAbortError } from 'remote-savant-mcp';
...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['REMOTE_SAVANT_MCP_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: RemoteSavantMcp) => {
    ...
    test('debug logs when log level is debug', async () => {
      ...
      const client = new RemoteSavantMcp({ logger: logger, logLevel: 'debug', apiKey: 'My API Key' });
    ...
    test('debug logs are skipped when log level is info', async () => {
      ...
      const client = new RemoteSavantMcp({ logger: logger, logLevel: 'info', apiKey: 'My API Key' });
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['REMOTE_SAVANT_MCP_LOG'] = 'debug';
      const client = new RemoteSavantMcp({ logger: logger, apiKey: 'My API Key' });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['REMOTE_SAVANT_MCP_LOG'] = 'not a log level';
      const client = new RemoteSavantMcp({ logger: logger, apiKey: 'My API Key' });
      expect(client.logLevel).toBe('warn');
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'REMOTE_SAVANT_MCP_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
    ...
    test('client log level overrides env var', async () => {
      ...
      process.env['REMOTE_SAVANT_MCP_LOG'] = 'debug';
      const client = new RemoteSavantMcp({ logger: logger, logLevel: 'off', apiKey: 'My API Key' });
  ...
  describe('baseUrl', () => {
    ...
    afterEach(() => {
      process.env['REMOTE_SAVANT_MCP_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new RemoteSavantMcp({ baseURL: 'https://example.com', apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com');
    });

    test('env variable', () => {
      process.env['REMOTE_SAVANT_MCP_BASE_URL'] = 'https://example.com/from_env';
      const client = new RemoteSavantMcp({ apiKey: 'My API Key' });
  ...
  test('maxRetries option is correctly set', () => {
    const client = new RemoteSavantMcp({ maxRetries: 4, apiKey: 'My API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new RemoteSavantMcp({ apiKey: 'My API Key' });
  ...
  test('with environment variable arguments', () => {
    ...
    process.env['REMOTE_SAVANT_MCP_API_KEY'] = 'My API Key';
    const client = new RemoteSavantMcp();
  ...
  test('with overridden environment variable arguments', () => {
    // set options via env var
    process.env['REMOTE_SAVANT_MCP_API_KEY'] = 'another My API Key';
    const client = new RemoteSavantMcp({ apiKey: 'My API Key' });
    expect(client.apiKey).toBe('My API Key');
  });
});
...
describe('retries', () => {
  test('retry on timeout', async () => {
    ...
    const client = new RemoteSavantMcp({ apiKey: 'My API Key', timeout: 10, fetch: testFetch });
  ...
  test('retry count header', async () => {
    ...
    const client = new RemoteSavantMcp({ apiKey: 'My API Key', fetch: testFetch, maxRetries: 4 });
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new RemoteSavantMcp({ apiKey: 'My API Key', fetch: testFetch });
